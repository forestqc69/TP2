...
void ReseauInterurbain::chargerReseau(std::ifstream & fichierEntree)
	...
	while (getline(fichierEntree, buff))
	{
		...
		unReseau.ajouterArc(unReseau.getNumeroSommet(source), unReseau.getNumeroSommet(destination),
				duree, cout);
	}
}
...
TP2::ReseauInterurbain::ReseauInterurbain(std::string nomReseau, size_t nbVilles)
		{
}

TP2::ReseauInterurbain::~ReseauInterurbain()
{
}

void TP2::ReseauInterurbain::resize(size_t nouvelleTaille)
		{
}

Chemin TP2::ReseauInterurbain::rechercheCheminDijkstra(const std::string& source,
		const std::string& destination, bool dureeCout) const
		{
}

std::vector<std::vector<std::string> > TP2::ReseauInterurbain::algorithmeKosaraju()
{
}
